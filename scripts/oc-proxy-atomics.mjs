 ...
 *
 * Usage:
 *   ./scripts/oc-proxy-atomics.sh              # all meshes: record→kill→revive→CONTINUE
 *   node scripts/oc-proxy-atomics.mjs record pia secretary
 *   node scripts/oc-proxy-atomics.mjs roundtrip pia secretary
 *   node scripts/oc-proxy-atomics.mjs roundtrip pia all-oc-proxy
 */
...
const STAMP = process.env.OC_PROXY_SES_STAMP || "/tmp/seatmesh-oc-proxy-sessions.json";
...
function wantOcProxy(entry) {
  ...
  return entry.type === "oc-proxy" || isOpenCodeCpeResumeCmd(entry.resume_cmd);
}
...
function listTargets(meshKey, which) {
  ...
  for (const p of panes) {
    ...
    if (which === "all-oc-proxy") {
      if (!wantOcProxy(entry)) continue;
    } else if (p.label !== which && p.label !== `slot-${which}` && p.label !== which) {
      continue;
    }
...
function revive(meshKey, which, stampPath = STAMP) {
  ...
  for (const row of stamp.seats) {
    ...
    if (which !== "all-oc-proxy" && row.label !== which) continue;
    if (!row.ses) {
...
if (!PROFILES[meshKey]) {
  console.error(`usage: record|kill|revive|roundtrip <pia|zsign|seatmesh> <secretary|all-oc-proxy|slot-N>`);
  process.exit(2);
}
...
if (action === "record") {
  ...
  const stamp = fs.existsSync(STAMP)
    ? JSON.parse(fs.readFileSync(STAMP, "utf8"))
    : record(meshKey, which);
  const rows = stamp.seats.filter((s) => s.mesh === meshKey && (which === "all-oc-proxy" || s.label === which));
