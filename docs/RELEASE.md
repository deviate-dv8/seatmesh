...
|--------------------|---------|--------|
| **2026-09-23** | `seatmesh@1.3.2` | Fixes `sm agent spawn`/fast-switch to `opencode`/`opencode-cpe`: the pasted launch line was `env -u NO_COLOR -u FORCE_COLOR COLORTERM=truecolor cd <workspace> && opencode --auto` — `env` execs its first bare arg as a binary, and `cd` isn't one, so this always failed with `env: 'cd': No such file or directory` the moment the target workspace differed from the pane's cwd. Reported live by the operator; fixed by putting `cd` before the `env` prefix, live-verified by replaying the exact repro in a real pane. |
| **2026-09-23** | `seatmesh@1.3.1` | `sm sessions kill <id\|label\|sessionName\|profilePath\|workspace>` (aliases `stop`/`down`) — kill any registered session's tmux + daemon without attaching, from the CLI or the web hub's new "Danger zone" panel on a session's Ops page. Operator-only (blocked for in-pane agents). `findGlobalSession`/`forgetGlobalSession` now also match by label, and `forgetGlobalSession` by sessionName (both were missing before). |
