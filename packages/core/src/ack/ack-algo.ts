...
export function remindableAcks(_rows: AckRow[]): AckRow[] {
  // Never re-inject. First ask paste is the only prompt; whoami/banner list open
  // rows; `ack <id>` closes without a peer reply (n+1 was burning turns).
  return [];
}
